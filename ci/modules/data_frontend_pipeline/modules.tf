...
locals {
  ...
  "${var.project_name}-codebuild-deploy-integration", "${var.project_name}-codebuild-deploy-staging", "${var.project_name}-codebuild-check-integration-restart", "${var.project_name}-codebuild-deploy-production"]
}

module "codepipeline_iam" {
  source                  = "../codepipeline_iam"
  project_name            = "data-frontend"
  region                  = var.region
